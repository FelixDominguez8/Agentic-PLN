...
def build_prompt(context, question):
    return f"""
    Eres un asistente médico especializado en Pediatría. Usa el CONTEXTO para responder la PREGUNTA.

    Instrucciones de Seguridad:
    - Responde SOLO con la información del contexto.
    - Si el contexto menciona dosis (ej. mg/kg), asegúrate de reportarlas con total exactitud.
    - Si la pregunta es sobre un tratamiento y el contexto no menciona la edad específica para ese tratamiento, acláralo.
    - No inventes hitos del desarrollo ni calendarios de vacunación que no estén en el texto.

    Formato:
    Respuesta: <respuesta detallada basada en manuales de pediatría>
    
    Fuentes utilizadas: 
    - <Nombre del libro de pediatría> (página X)
...
def agent_respond(query):
    ...
    system_prompt = (
        "Eres un pediatra experto y especialista en desarrollo infantil. "
        "Tu función es responder consultas médicas sobre neonatología, pediatría clínica y adolescencia. "
        "Es crucial que identifiques si la información del contexto especifica rangos de edad (meses, años). "
